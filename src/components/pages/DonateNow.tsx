...
const DonateNow = () => {
  ...
  const [createDonation, { isLoading: isDonating }] =
  ...
  const campaignDetails = [
    ...
    {
      label: "Category",
      value: campaign?.campaignCategory || "N/A",
      icon: Box,
    },
    {
      label: "Campaign Code",
      value: campaign?.campaignCode || "N/A",
      icon: CalendarDays,
    },
    {
      label: "Delivery Options",
      value: campaign?.deliveryOptions?.shipping ? "Shipping" : "Standard",
      icon: Truck,
    },
    {
      label: "Shipping Fee",
      value: `$${campaign?.shippingFee || 0}`,
      icon: ShoppingCart,
    },
  ] as const;
  ...
  const handleDonateSubmit = async (
    ...
    try {
      ...
      const errMsg =
        err?.data?.message || "Failed to initiate donation. Please try again.";
      setError(errMsg);
      toast.error(errMsg);
    }
  };
    ...
    return (
      <main className="bg-background py-20">
        <div className="container mx-auto flex flex-col items-center justify-center gap-4 text-center">
          ...
          <Link
            href="/campaign"
            className="mt-2 inline-flex h-10 items-center justify-center rounded-lg bg-secondary px-6 text-sm font-semibold text-white transition-all hover:bg-secondary/90"
          >
            Browse Campaigns
          </Link>
        </div>
        ...
        <div className="mx-auto mt-6 grid max-w-6xl items-start gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <aside className="rounded-xl border border-muted-foreground/60 bg-white p-4 sm:p-5">
            ...
            <div className="relative mt-4 aspect-[1.52/1] overflow-hidden rounded-xl bg-slate-100">
              <Image
                src={campaign.thumbnail || orderImage}
                alt={campaign.name}
                fill
                priority
            ...
            <div className="mt-4 flex items-center gap-4">
              <div className="relative size-12 shrink-0 overflow-hidden rounded-full border-2 border-white shadow-sm bg-slate-100">
                <Image
                  src={campaign.organizer?.profileImage || campaignOwner}
                  alt={campaign.name}
                  fill
                  className="object-cover"
                  sizes="48px"
                  unoptimized={!!campaign.organizer?.profileImage}
                />
              </div>
              <h2
                className="text-lg leading-6 font-semibold truncate max-w-[200px]"
                title={campaign.name}
              >
                {campaign.name}
              </h2>
            ...
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-secondary/15">
              <div
                ...
                style={{
                  width: `${Math.min(100, Math.round(((campaign.raisedAmount || 0) / (campaign.goalAmount || 1)) * 100))}%`,
                }}
              />
            </div>
            ...
            <Link
              ...
              className="mt-4 flex h-11 items-center justify-center gap-2 rounded-lg bg-primary text-sm font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-primary/90 hover:shadow-md"
            >
              ...
              <dl className="mt-4 space-y-3">
                {campaignDetails.map((detail) => {
                  ...
                  return (
                    <div
                      key={detail.label}
                      className="grid grid-cols-[1fr_1.1fr] items-start gap-3 text-xs"
                    >
          ...
          <div className="space-y-5">
            <section>
              ...
              <div className="mt-3 rounded-xl border border-muted-foreground/60 p-4 text-sm leading-6 text-muted-foreground sm:p-5">
                <p className="whitespace-pre-line">
                  {campaign.story || "No story description provided."}
                </p>
              </div>
            </section>
            ...
            <form className="space-y-5" onSubmit={handleDonateSubmit}>
              <section className="rounded-xl border border-muted-foreground/60 p-5 sm:p-6 bg-white">
                ...
                {error && (
                  <p className="mt-3 text-xs font-semibold text-red-500 bg-red-50 p-2.5 rounded-lg border border-red-100">
                    {error}
                  </p>
                )}
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    <FormField
                      label="Message"
                      name="message"
                      placeholder="Keep up the great work! Happy to support."
                      required
                    />
                  </div>
                  <div className="sm:col-span-2">
              ...
              <section className="rounded-xl border border-muted-foreground/60 p-5 sm:p-6 bg-white">
                ...
                <button
                  type="submit"
                  disabled={isDonating}
                  className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-secondary text-sm font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-secondary/90 hover:shadow-md cursor-pointer"
                >
                  {isDonating ? (
