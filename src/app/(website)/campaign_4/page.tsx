...
const summary = [
  ["Campaign Name", "Jenna’s Banana Pudding"],
  ["Goal", "$2,500"],
  ["Price", "$10 each"],
      ...
      <div className="container mx-auto">
        <ol aria-label="Campaign creation progress" className="mx-auto flex max-w-3xl items-start">
          {steps.map((step, index) => {
            ...
            return (
              <li key={step} className={`relative flex flex-1 flex-col items-center text-center ${index < steps.length - 1 ? "after:absolute after:left-1/2 after:top-5 after:-z-0 after:h-px after:w-full after:bg-secondary" : ""}`}>
                ...
                <span className={`relative z-10 mt-3 bg-white px-2 text-sm font-medium sm:text-base ${complete ? "text-secondary" : "text-primary"}`}>{step}</span>
              </li>
            );
          })}
        </ol>
        ...
        <div className="mx-auto mt-14 w-full max-w-6xl lg:mt-20">
          <header>
            <span className="inline-flex bg-secondary/10 px-3 py-1.5 text-base font-medium text-secondary">Step 4 of 4</span>
            <div className="mt-5 flex items-start gap-5">
              <span className="hidden size-20 shrink-0 items-center justify-center rounded-full bg-secondary/10 text-secondary sm:flex"><Rocket className="size-10" /></span>
              <div><h1 className="text-[32px] font-semibold leading-tight tracking-tight text-black">Your Campaign is Ready!</h1><p className="mt-3 max-w-2xl text-lg leading-7 text-muted-foreground">Review your campaign below. If everything looks good, launch and start accepting orders and donations.</p></div>
            </div>
          </header>

          <div className="mt-10 grid items-start gap-10 lg:grid-cols-[1.15fr_0.85fr] xl:gap-16">
            <section>
              <h2 className="flex items-center gap-3 text-[32px] font-semibold leading-tight"><Sparkles className="size-10 text-secondary" />Campaign Preview</h2>
              ...
              <p className="mt-5 text-center text-sm text-muted-foreground">This is exactly how supporters will see your campaign.</p>
            </section>

            <form onSubmit={handleLaunch} className="space-y-6 lg:sticky lg:top-6">
              <Panel title="Campaign Summary" icon={CheckCircle2}>
                <dl className="space-y-3">{summary.map(([label, value]) => <div key={label} className="grid grid-cols-[1fr_1.1fr] gap-3 text-sm"><dt className="flex items-center gap-2 text-muted-foreground"><Check className="size-4 text-secondary" />{label}</dt><dd className="text-right font-medium">{value}</dd></div>)}</dl>
              </Panel>

              <Panel title="Ready To Launch!" icon={Rocket} tone="orange">
                <p className="text-sm leading-6 text-muted-foreground">Your campaign has everything needed and is ready to go live.</p><ul className="mt-4 space-y-3">{readiness.map((item) => <li key={item} className="flex items-start gap-2 text-sm"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-secondary" />{item}</li>)}</ul>
              </Panel>
              ...
              <Panel title="What Happens Next?" icon={Clock3}>
                <ul className="space-y-3">{["Campaign goes live instantly", "Receive orders and donations", "Track supporters", "Download customer spreadsheet", "Get paid when campaign ends"].map((item) => <li key={item} className="flex items-start gap-2 text-sm"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-secondary" />{item}</li>)}</ul>
              </Panel>
          ...
          <section className="mt-14 flex flex-col items-center gap-6 rounded-lg bg-secondary/10 p-7 text-center sm:flex-row sm:text-left"><span className="flex size-20 shrink-0 items-center justify-center rounded-full bg-white text-secondary"><Sparkles className="size-10" /></span><div className="flex-1"><h2 className="text-[32px] font-semibold leading-tight">Need More Time?</h2><p className="mt-2 text-lg leading-7 text-muted-foreground">Your campaign draft is automatically saved. Launch whenever you’re ready.</p></div><div className="flex items-center gap-4 text-secondary"><Gift className="size-10" /><ShieldCheck className="size-10" /></div></section>
        </div>
      </div>
    </main>
  );
...
function Panel({ title, icon: Icon, tone = "teal", children }: { title: string; icon: React.ComponentType<{ className?: string }>; tone?: "teal" | "orange"; children: React.ReactNode }) {
  return <section className={`rounded-lg border p-5 ${tone === "orange" ? "border-primary/60 bg-orange-50/40" : "border-slate-400 bg-white"}`}><h2 className={`mb-4 flex items-center gap-3 text-lg font-semibold ${tone === "orange" ? "text-primary" : "text-foreground"}`}><Icon className={`size-6 ${tone === "orange" ? "text-primary" : "text-secondary"}`} />{title}</h2>{children}</section>;
}

function FeeRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return <div className={`flex justify-between gap-4 ${strong ? "border-t border-secondary/30 pt-3 font-semibold text-secondary" : ""}`}><dt>{label}</dt><dd>{value}</dd></div>;
}
