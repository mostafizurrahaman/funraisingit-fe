...
export default function CampaignFivePage() {
  ...
  async function copyLink(type: "link" | "share") {
    try {
      ...
      setCopyError(
        "Could not copy automatically. Please copy the campaign URL manually.",
      );
    }
  }
      ...
      <div className="container mx-auto">
        <div className="mx-auto w-full max-w-6xl">
          ...
          <section className="relative mt-8 overflow-hidden py-8 text-center">
            ...
            <h1 className="relative z-10 mt-5 text-[32px] font-semibold leading-tight tracking-tight text-black">
              🎉 Congratulations!
            </h1>
            <p className="relative z-10 mt-3 text-lg font-semibold text-secondary">
              Jenna’s Banana Pudding Is Now Live!
            </p>
            <p className="relative z-10 mt-2 text-lg leading-7 text-muted-foreground">
          ...
          <section className="mt-5 rounded-lg border border-secondary bg-secondary/10 p-5 sm:p-6">
            <div className="flex flex-col items-center gap-5 sm:flex-row">
              ...
              <div className="min-w-0 flex-1 text-center sm:text-left">
                <p className="text-lg font-semibold">Your Campaign Link:</p>
                <p className="mt-1 truncate text-lg font-medium text-secondary">
                  funraisingit.com/jennas-banana-pudding
                </p>
              </div>
              <Button
                type="button"
            ...
            {copyError ? (
              <p role="alert" className="mt-3 text-center text-lg text-red-600">
                {copyError}
              </p>
            ) : null}
          </section>
          ...
          <section className="mt-10 text-center">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-5">
              ...
              <ShareButton
                ...
                onClick={() => {
                  window.location.href = `sms:?body=${encodeURIComponent(`Support Jenna’s Banana Pudding: ${campaignUrl}`)}`;
                }}
              />
              <ShareButton
                icon={Mail}
                ...
                color="text-red-500"
                onClick={() => {
                  window.location.href = `mailto:?subject=${encodeURIComponent("Support Jenna’s Banana Pudding")}&body=${encodeURIComponent(campaignUrl)}`;
                }}
              />
              <ShareButton
                icon={FaWhatsapp}
                ...
                onClick={() =>
                  openShare(
                    `https://wa.me/?text=${encodeURIComponent(`Support Jenna’s Banana Pudding: ${campaignUrl}`)}`,
                  )
                }
              />
              <ShareButton
            ...
            <div className="space-y-7">
              ...
              <Panel title="First Supporter Challenge" icon={Sparkles}>
                <p className="text-lg leading-7 text-muted-foreground">
                  Most successful campaigns receive their first supporters
                  within 24 hours.
                </p>
                <div className="mt-6 flex flex-col items-center gap-5 sm:flex-row">
                  <div className="flex gap-3">
                    {Array.from({ length: 5 }, (_, index) => (
                      <span
                        key={index}
                        className="size-10 rounded-full border-2 border-slate-400"
                      />
                    ))}
                  </div>
                ...
                <p className="mt-5 flex items-center gap-2 text-lg">
                  ...
                  <strong>Reward:</strong> Unlock your First Campaign Success
                  Badge
                </p>
              </Panel>
              ...
              <Panel title="What Would You Like To Do Next" icon={PartyPopper}>
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Button
                    ...
                    className="h-auto min-h-14 border-secondary text-secondary"
                  >
            ...
            <aside className="space-y-7 lg:sticky lg:top-6">
                ...
                <ul className="space-y-3">
                  {tips.map((tip, index) => (
                    <li key={tip} className="flex items-start gap-3 text-lg">
                      <span
                        className={`mt-1 flex size-5 shrink-0 items-center justify-center rounded-full text-white ${index === 0 ? "bg-blue-500" : index === 1 ? "bg-green-500" : index === 2 ? "bg-pink-500" : index === 3 ? "bg-primary" : "bg-purple-500"}`}
                      >
              ...
              <Panel title="Your Campaign Preview" icon={Heart}>
                <div className="flex gap-4">
                  <Image
                    src={hero}
                    alt="Banana pudding campaign"
                    className="size-28 rounded-lg object-cover object-right"
                  />
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <p className="font-semibold">Jenna’s Banana Pudding</p>
                    </div>
                    <p className="mt-3 text-sm font-semibold text-secondary">
                      Goal: $2,500
                    </p>
                    <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-secondary/15">
                      <div className="h-full w-[3%] bg-secondary" />
                    </div>
                    <div className="mt-2 flex justify-between text-xs">
                      <span>$80 Raised</span>
                      <span>0 Supporters</span>
                    </div>
                    <p className="mt-2 text-xs text-secondary">7 Days Left</p>
                  </div>
                </div>
          ...
          <section className="relative mt-12 overflow-hidden rounded-lg bg-secondary/10 p-7 sm:p-9">
            <Image
              src={price}
              alt="Campaign success reward"
              className="absolute bottom-0 left-3 hidden h-auto w-36 sm:block"
            />
            <Image
              src={like}
              alt="Supporter appreciation"
              className="absolute bottom-0 right-3 hidden h-auto w-36 sm:block"
            />
            <div className="relative z-10 mx-auto max-w-2xl text-center">
              <Trophy className="mx-auto size-10 text-yellow-500" />
              <h2 className="mt-3 text-[32px] font-semibold leading-tight text-secondary">
                Most successful campaigns get their first 5 supporters within
                the first 24 hours.
              </h2>
...
function ShareButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex min-h-24 flex-col items-center justify-center gap-2 rounded-lg border bg-white p-3 text-sm font-medium transition-all duration-300 hover:-translate-y-0.5 hover:border-secondary hover:shadow-md ${highlighted ? "border-primary bg-orange-50" : "border-slate-300"}`}
    >
...
function Panel({
  ...
  return (
    <section
      className={`rounded-lg border p-5 sm:p-6 ${tone === "orange" ? "border-primary/70 bg-orange-50/40" : "border-slate-300 bg-white"}`}
    >
      <h2 className="mb-5 flex items-center gap-3 text-[32px] font-semibold leading-tight">
        <Icon
          className={`size-10 ${tone === "orange" ? "text-primary" : "text-secondary"}`}
        />
        {title}
      </h2>
      {children}
...
function DollarIcon({ className }: { className?: string }) {
  return (
    <span
      className={`flex items-center justify-center rounded-full border border-secondary font-semibold ${className}`}
    >
      $
    </span>
  );
