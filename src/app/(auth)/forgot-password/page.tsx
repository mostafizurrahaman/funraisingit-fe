  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#26343a] px-4 py-8">
      <section className="w-full max-w-[360px] rounded-lg bg-white px-5 py-10 shadow-xl">
        ...
        <div className="mt-6 text-center"><h1 className="text-2xl font-semibold tracking-tight">Email Confirmation</h1><p className="mt-0.5 text-[11px]">Enter Your email for verification.</p></div>
        <form onSubmit={handleSubmit} className="mt-3.5 space-y-6">
          <div><label htmlFor="email" className="mb-1.5 block text-sm font-medium">Email</label><input id="email" name="email" type="email" placeholder="you@example.com" autoComplete="email" required className="h-11 w-full rounded-lg border border-slate-500 bg-white px-3 text-sm outline-none transition-all duration-300 placeholder:text-slate-500 focus:border-secondary focus:ring-2 focus:ring-secondary/20" /></div>
          <Button type="submit" disabled={isPending} className="h-9 w-full rounded-lg text-xs">{isPending ? "Please wait..." : "Register"}</Button>
        </form>
      </section>
    </main>
  );
